...
from .ngram import StreamingQuantizedEmbedding
...
def load_streaming(
    ...
    if verbose:
        ...
        print(
            f"  residente : {len(resident)} tensores, "
            f"{sum(_nbytes(store, k) for k in resident)/1e9:.2f} GB"
        )
        print(f"  streaming : {streamed_bytes/1e9:.2f} GB")
        print(f"  trocados  : {swapped_experts} blocos MoE, {swapped_ngram} shards n-gram")
        print(f"  MTP       : {'ativo, especialistas no SSD' if use_mtp else 'inativo'}")
    ...
    for index, layer in enumerate(layers):
        ...
        for shard_index in range(len(table.shards)):
            ...
            if f"{prefix}.weight" not in store.refs:
                continue
            table.shards[shard_index] = StreamingQuantizedEmbedding(
