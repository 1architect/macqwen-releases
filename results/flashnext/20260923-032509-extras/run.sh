...
arm() {
  echo "=== arm $1: $2" | tee -a results/flashnext/20260923-032509-extras/output.log
  if [[ $2 == extras ]]; then
    env $EXTRAS $PY -m models.flashnext.tests.bench.bench_long_states --tokens 128 --window 64 \
      --conditions defaults --json results/flashnext/20260923-032509-extras/arm-$1-extras.json 2>&1 | tee -a results/flashnext/20260923-032509-extras/output.log
  else
    $PY -m models.flashnext.tests.bench.bench_long_states --tokens 128 --window 64 \
      --conditions defaults --json results/flashnext/20260923-032509-extras/arm-$1-defaults.json 2>&1 | tee -a results/flashnext/20260923-032509-extras/output.log
  fi
}
arm 1 defaults
arm 2 extras
arm 3 extras
arm 4 defaults
echo "=== done" | tee -a results/flashnext/20260923-032509-extras/output.log
