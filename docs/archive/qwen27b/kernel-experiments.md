# Archived V3.1 kernel experiments

Archive status: rejected. Quantized matrix multiplication already reached the chip limit.

## V3.1 experimental prefill mode benchmark

This historical protocol compared candidate kernels with the `64/32/64` QMM. It measured raw neural prefill.
Prefix-cache load speed used a separate metric.
...
The protocol required a separate 8 GB wired-limit test. It kept that result separate from the server result.
...
A fused epilogue must preserve the baseline BF16 rounding points. Otherwise, classify it as an approximate mode and use
the separate quality gate below.

### Stage 1: component speed

Measure each projection and the complete MLP. Report median milliseconds, paired speedup, active memory, peak memory,
and cache memory.
...
Stop a branch that misses its continuation gate. Small component gains cannot produce a large full-model gain.

### Stage 2: cold full-model speed

The cold test used AC power, a cool hard surface, and an idle Mac. It warmed kernel compilation only. It ran nine
interleaved pairs with 256 real repository tokens and equal caches.
Report:
...
The reference was approximately 50 tok/s before thermal throttling.

### Stage 3: hot sustained speed

Advance two equal prompt caches, one per path. Process the same 256-token chunks in alternating order for 15 minutes. Do
not reset the thermal state.
Report one-minute windows. Define hot speed as the median of the last five minutes. Also report:
...
The reference was approximately 40 tok/s when hot.
Run context checkpoints at 0, 4096, 8192, and 16384 cached tokens. This finds an improvement that helps short MLP work
but loses to long-context attention.
After the paired test, run each path alone for 15 minutes. Randomize their order across four cooled starts. Start again
only when the baseline probe is within 2% of its original cold rate. This detects extra candidate heat that can slow
both paths during an interleaved test.
...
For acceptance, the lower 95% confidence bound must exceed the selected target. Candidate hot retention must not trail
baseline retention by more than two percentage points.
...
The old 256-token result was 169.05 ms for QMM and 150.01 ms for persistent BF16 GEMM. The dense comparator was 1.127x
faster. This was a conservative LUT estimate, not a physical limit. A Q2-only optimization covers 162 of 192 equal-size
MLP projections.
...
The new `64/32/64` baseline can reduce this remaining headroom. Re-measure it before implementing a LUT. Treat LUT Q2 as
a supporting optimization.

### Gate, up, and SwiGLU fusion

Gate and up contain two thirds of MLP matrix work. If the fusion accelerates only that subgraph by `s`:
...
For 256 tokens, fusion avoids at least 35.65 MB of gate/up intermediate traffic per layer. Across 64 layers, this is
2.28 GB per chunk.

### Tensor operations

The Metal QMM already used Steel `BlockMMA`. MLX NAX required a later architecture than M4.
A different matrix path must beat the complete MLP by 1.25x before full-model testing. CPU and GPU co-processing must
include synchronization, dequantizing, shared-memory traffic, and sustained thermal cost in its timing.
...
Use the same tokenizer, prompt, sampling, and token limit. Pin benchmark commits. Report paired task outcomes, not only
aggregate scores.
Reject an approximate mode when any safety or tool-format regression appears.

### Required result record

Save each experiment under `v31_work/` with a `prefill_mode_` prefix. Include:
...
Device: Apple M4 GPU. Projection weights: real V3.1 layer-0 affine Q2. Activation estimate: symmetric A8 per token and
group of 128 values.
...
The raw integer result matches the reference exactly. It omits affine weight scales, weight biases, A8 scales, and group
corrections. Those operations only increase its final time.
Stop result: reject A8W2 TensorOps on M4. The raw arithmetic is about 1.42x slower than the current QMM before required
epilogue work.
...
It loads only `model-layer-00.safetensors`. It does not patch MLX, the model, the runtime, or a launcher.

### Method

Each packed byte contains four Q2 codes. The kernel creates a 16-entry subset sum table from the corresponding four BF16
activations. It stores only eight entries and reconstructs the other eight with the complement identity.
...
The codebook variant also preserves the four BF16-rounded dequantization levels before it reorganizes the sum.
...
```text
~/mlx-qwen38-kernel-lab/bin/python \
...
The affine kernel is mathematically exact for the stored affine parameters. It is not bitwise identical because it
changes the floating-point reduction order. The projection-level error is small.

### Decision

This scalar Metal LUT design is not viable. It is 6.8x slower at 64 tokens. It is 16.4x slower at 256 tokens.
The M4 SIMD matrix unit makes dequantize-plus-MMA much faster than scalar threadgroup lookups. LUT methods that help CPU
decode do not transfer directly to a large GPU prefill batch.
