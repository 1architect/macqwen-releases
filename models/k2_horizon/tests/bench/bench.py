...
def checkpoint_info(requested: str) -> dict[str, Any]:
    try:
        ...
        path = resolve_k2_horizon(requested)
...
def source_fingerprints() -> dict[str, str | None]:
    names = ("models/k2_horizon/backend.py", "models/k2_horizon/cache.py", "models/k2_horizon/checkpoint.py", "models/k2_horizon/protocol.py", "models/k2_horizon/settings.py")
...
def child_arm(*, checkpoint: str, arm_id: str, condition: str, options: dict[str, Any],
    ...
    try:
        ...
        backend = K2HorizonBackend(checkpoint, prefill_step_size=effective_prefill_step, **constructor)
        import mlx.core as mx
        mx.random.seed(seed)
        record["snapshots"]["load"] = snapshot(backend, "load")
        if options.get("cache_step") is not None:
            from models.k2_horizon.cache import set_kv_cache_step
...
def run_comparison(*, checkpoint: str, comparison: str, record_path: str, fixture="context-2k", horizon=SHORT, window=WINDOW, rounds=3,
    ...
    for round_index, condition in ordered_conditions(list(conditions), rounds):
        ...
        command = [sys.executable, "-m", "models.k2_horizon.tests.bench.bench", "--child",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--checkpoint", default=os.environ.get("MACQWEN_K2_HORIZON_MODEL", "k2"))
    ...
    args = parser.parse_args(argv)
    args.jsonl = str(output_path("k2_horizon", "bench", "arms.jsonl", args.jsonl))
