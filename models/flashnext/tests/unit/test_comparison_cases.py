...
from macqwen.testsuite.api import TestContext, TestSpec
...
class ComparisonCaseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.catalog = build_catalog("flashnext")
        cls._run_dir = tempfile.TemporaryDirectory()
        cls.config = TestContext("flashnext", "", "python")
        cls.config.run_dir = Path(cls._run_dir.name)

    @classmethod
    def tearDownClass(cls):
        cls._run_dir.cleanup()
    ...
    def test_product_long_runs_one_current_answer(self):
        ...
        command = spec.script(self.config, None)
        self.assertIn("models.flashnext.tests.bench.bench_product_long", command)
