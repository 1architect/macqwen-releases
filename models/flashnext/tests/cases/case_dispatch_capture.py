...
def command(config, result_path: Path) -> list[str]:
    ...
    return [str(config.python), "-m", bench_module("flashnext", "capture_dispatches"), "--out", str(capture), "--tokens", "2"]
...
TEST = TestSpec(
    ...
    source="models/flashnext/tests/bench/capture_dispatches.py",
)
