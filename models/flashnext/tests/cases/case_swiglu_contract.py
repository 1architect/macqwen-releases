...
from macqwen.testsuite.api import TestSpec


def command(config, _result_path: Path) -> list[str]:
    return [str(config.python), "-m", "models.flashnext.tests.cases.case_swiglu_contract", "--execute"]


def main(argv=None) -> int:
    if "--execute" not in (argv or sys.argv[1:]):
        return 0
    from models.flashnext.tests.bench.swiglu_contract import exhaustive_gate_patterns
...
TEST = TestSpec(
    ...
    source="models/flashnext/tests/bench/swiglu_contract.py", canonical=False,
)
