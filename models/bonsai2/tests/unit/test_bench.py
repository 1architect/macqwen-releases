...
from models.bonsai2.tests.bench import bench
...
class BenchTests(unittest.TestCase):
    ...
    def test_runnable_prefill_catalog_is_bounded(self):
        ...
        for test_id in bounded:
            ...
            command = spec.script(SimpleNamespace(
                ...
                output=lambda name: Path("run") / name,
            ), Path("run/record.jsonl"))
            ...
            **base,
            "runtime_source_fingerprints": {
                **base["runtime_source_fingerprints"],
                "models/bonsai2/tests/bench/bench.py": "original",
            },
        }
        changed = {
            **manifest,
            "runtime_source_fingerprints": {
                **manifest["runtime_source_fingerprints"],
                "models/bonsai2/tests/bench/bench.py": "changed",
            },
        }
        calls = []
    ...
    def test_run_comparison_keeps_raw_rows_and_records_digest_failures(self):
        ...
        self.assertEqual(calls[0][1:4], ["-m", "models.bonsai2.tests.bench.bench", "--child"])
    ...
    def test_main_reports_failure_through_its_exit_code(self):
        ...
        self.enterContext(patch.object(results, "RESULTS_ROOT", Path(directory)))
        record = str(Path(directory) / "x.jsonl")
        with patch.object(bench, "run_comparison",
                          return_value={"status": "completed"}) as run:
            self.assertEqual(
                bench.main(["--compare", "baseline", "--jsonl", record]), 0
            )
        run.assert_called_once()
        with patch.object(bench, "run_comparison",
                          return_value={"status": "completed_with_failures"}):
            self.assertEqual(
                bench.main(["--compare", "baseline", "--jsonl", record]), 1
            )
        with patch.object(bench, "run_comparison",
                          return_value={"status": "interrupted"}):
            self.assertEqual(
                bench.main(["--compare", "baseline", "--jsonl", record]), 130
            )
